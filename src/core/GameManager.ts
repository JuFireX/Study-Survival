...
export class GameManager {
  ...
  private onGamePause(): void {
    if (this.pauseCount === 0) {
      this.previousTimeScale =
        typeof this.app.timeScale === "number" ? this.app.timeScale : 1;
  ...
  public startLobby() {
    ...
    this.lobbyCharacterStand =
      characterStand instanceof pc.Entity ? characterStand : null;
  ...
  private moveLobbyPlayer(direction: pc.Vec3, dt: number) {
    ...
    const newRotation = new pc.Quat().slerp(
      currentRotation,
      targetRotation,
      10 * dt,
    );
  ...
  private updateLobbyActionState() {
    ...
    if (this.lobbyPortal) {
      canEnterPortal =
        playerPos.distance(this.lobbyPortal.getPosition()) <=
        this.lobbyInteractRadius;
    }

    if (this.lobbyCharacterStand) {
      canOpenCharacter =
        playerPos.distance(this.lobbyCharacterStand.getPosition()) <=
        this.lobbyInteractRadius;
    }
