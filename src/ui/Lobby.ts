...
export class Lobby {
  ...
  private onPointerDown(e: PointerEvent) {
    ...
    if (
      this.characterModule.isSelectionVisible() ||
      this.skillStand.isPanelVisible() ||
      this.equipmentStand.isPanelVisible()
    )
      return;
    if (
    ...
    const portalHit =
      portal && this.canEnterPortal
        ? this.raycastSphere(ray, portal.getPosition(), this.portalRadius)
        : null;
    const standHit =
      stand && this.canOpenCharacter
        ? this.raycastSphere(ray, stand.getPosition(), this.characterRadius)
        : null;
    const skillHit = skillStand
      ? this.raycastSphere(ray, skillStand.getPosition(), this.skillRadius)
      : null;
    const equipmentHit = equipmentStand
      ? this.raycastSphere(
          ray,
          equipmentStand.getPosition(),
          this.equipmentRadius,
        )
      : null;
    ...
    const hits = [
      ...
      {
        distance: skillHit,
        handler: () => this.skillStand.setPanelVisible(true),
      },
      {
        distance: equipmentHit,
        handler: () => this.equipmentStand.setPanelVisible(true),
      },
    ]
