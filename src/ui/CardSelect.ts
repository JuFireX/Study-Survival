...
import {
  ...
  CardFaceState,
  RarityWeight,
} from "./components/CardSelectComponent";
...
export class CardSelect {
  ...
  public start(
    ...
    callback: (selectedCardIds: string[]) => void,
  ) {
    this.onCompleted = callback;
    this.pairs = this.pairCards(questions, rewards);

    const questionFrontPairs = this.pairs.filter(
      (p) => p.front.type === CardType.Question,
    );
    if (
      questionFrontPairs.length > 0 &&
      questionFrontPairs.length < this.pairs.length
    ) {
      const nonQuestionFrontPairs = this.pairs.filter(
        (p) => p.front.type !== CardType.Question,
      );
  ...
  private handleFlip(pairId: string) {
    ...
    const currentCard =
      pair.currentFace === CardFaceState.Front ? pair.front : pair.back;
    ...
    // 执行翻转
    pair.currentFace =
      pair.currentFace === CardFaceState.Front
        ? CardFaceState.Back
        : CardFaceState.Front;
  ...
  private handleAnswer(pairId: string, answer: number | string) {
    ...
    const qCard = (
      pair.front.type === CardType.Question ? pair.front : pair.back
    ) as QuestionCard;
    ...
    if (typeof answer === "number") {
      ...
      const input =
        typeof rawInput === "string" ? rawInput.trim().toLowerCase() : "";
      ...
      if (input.length > 0 && typeof expectedRaw === "string") {
        ...
        isCorrect =
          candidates.length > 0
            ...
            : expectedRaw.trim().toLowerCase() === input;
      }
    }
