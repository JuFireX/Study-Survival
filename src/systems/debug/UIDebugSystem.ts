...
export class UIDebugSystem implements IGameSystem {
  ...
  update(dt: number): void {
    ...
    if (this.app.keyboard.isPressed(pc.KEY_J)) {
      ...
      if (joystick) {
        if (
          Math.abs(joystick.value.x) > 0.01 ||
          Math.abs(joystick.value.y) > 0.01
        ) {
    ...
    const questions: QuestionCard[] = [
      {
        ...
        question: { id: 1, subject: "Math", difficulty: 1, text: "1+1=?" },
      },
      {
        id: "q2",
        rarity: CardRarity.Rare,
        type: CardType.Question,
        question: { id: 2, subject: "Math", difficulty: 2, text: "2*2=?" },
      },
      {
        id: "q3",
        rarity: CardRarity.Epic,
        type: CardType.Question,
        question: { id: 3, subject: "Math", difficulty: 3, text: "3^2=?" },
      },
    ];
  ...
  private testFloatingText() {
    ...
    if (!camera) {
      console.warn(
        "No camera found for FloatingText test, texts might not appear correctly.",
      );
    }
