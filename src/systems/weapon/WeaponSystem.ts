...
import { IGameSystem, WeaponStats, Card, CardType, BuffCard, CardEffect } from '../../config/types';
...
export class WeaponSystem implements IGameSystem {
    ...
    private applyEffect(effect: CardEffect) {
        ...
        if (effect.target === 'w_*') {
            ...
            console.log(`[武器系统] 应用全局 Buff: ${effect.stat} ${effect.type} ${effect.value}`);
        } else {
            // 特定武器增益 (e.g., w_pistol)
            ...
            this.activeWeapons.forEach(w => {
                ...
                if (w.id.startsWith(effect.target)) {
                    ...
                    console.log(`[武器系统] 应用特定 Buff: ${w.id} ${effect.stat} ${effect.type} ${effect.value}`);
                }
            });
        }
    }
