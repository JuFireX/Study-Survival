...
export class CardSystem implements IGameSystem {
  ...
  private bindEvents(): void {
    ...
    eventBus.on(
      "card:selectionCompleted",
      (ids: string[]) => this.onSelectionCompleted(ids),
      this,
    );
  }
  ...
  private startSelectionFlow(): void {
    ...
    this.context
      .getEventBus()
      .fire("ui:showCardSelection", { questions, rewards });
  }
  ...
  private onSelectionCompleted(selectedCardIds: string[]): void {
    ...
    if (snapshot) {
      ...
      this.rewardDiscard.push(
        ...snapshot.rewards.filter((r) => !selectedSet.has(r.id)),
      );
    }
  ...
  private drawFromDeck<T extends Card>(
    deck: T[],
    discard: T[],
    pool: T[],
    count: number,
